...
public class LiveViewEngineTcpClientTests
{
    ...
    [Fact]
    public async Task IngestAsync_WaitsForConnectedHandshakeBeforeSendingRequests()
    {
        using var listener = new TcpListener(IPAddress.Loopback, 0);
        listener.Start();
        var port = ((IPEndPoint)listener.LocalEndpoint).Port;
        ...
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        var serverTask = RunFakeHandshakeServerAsync(listener, cts.Token);
        var client = new LiveViewEngineTcpClient(
            new LiveViewEngineTcpClientOptions
            {
                Host = "127.0.0.1",
        ...
        var runTask = client.RunAsync(cts.Token);
        var ingestTask = client.IngestAsync(
            "trades",
            "trade-1",
            new Dictionary<string, string?>
            {
    ...
    private static async Task RunFakeHandshakeServerAsync(TcpListener listener, CancellationToken ct)
    {
        using var acceptedClient = await listener.AcceptTcpClientAsync(ct).ConfigureAwait(false);
        await using var stream = acceptedClient.GetStream();
        using var reader = new StreamReader(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks: false, 1024, leaveOpen: true);
        ...
        var schemaLineTask = reader.ReadLineAsync(ct).AsTask();
        await Task.Delay(TimeSpan.FromMilliseconds(200), ct).ConfigureAwait(false);
        Assert.False(schemaLineTask.IsCompleted);

        await writer.WriteLineAsync(
            TcpProtocolCodec.SerializeResponse(new ConnectedResponseMessage(TcpProtocolCodec.ProtocolVersion)))
            .ConfigureAwait(false);

        var schemaLine = await schemaLineTask.ConfigureAwait(false);
        var schemaRequest = Assert.IsType<GetSchemaRequestMessage>(
            TcpProtocolCodec.ParseRequest(schemaLine ?? throw new InvalidOperationException("Missing schema request.")));
        Assert.Equal("trades", schemaRequest.CollectionName);
        ...
        var upsertLine = await reader.ReadLineAsync(ct).ConfigureAwait(false);
        var upsert = Assert.IsType<UpsertRequestMessage>(
            TcpProtocolCodec.ParseRequest(upsertLine ?? throw new InvalidOperationException("Missing upsert.")));
        var fields = upsert.Fields.ToDictionary(static field => field.Key, static field => field.Value);
        Assert.Equal("T000001", fields[1]);
        Assert.Equal("101.25", fields[2]);

        await writer.WriteLineAsync(
            TcpProtocolCodec.SerializeResponse(new AckResponseMessage(upsert.RequestId, "UPSERT")))
            .ConfigureAwait(false);
