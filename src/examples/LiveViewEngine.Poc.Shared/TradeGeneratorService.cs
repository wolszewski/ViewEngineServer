...
public class TradeGeneratorService(
    ITradeIngestionClient ingestionClient,
    ILogger<TradeGeneratorService> logger)
{
    ...
    private async Task RunGenerationAsync(TradeGenerationSettings settings, CancellationToken ct)
    {
        ...
        for (int index = 0; index < settings.InitialTradeCount; index++)
        {
            ...
            var success = await ingestionClient.IngestAsync(CollectionName, trade.Key, trade.Fields, ct);
            if (!success)
            {
    ...
    private static string GetFieldOrDefault(IReadOnlyDictionary<string, string?> fields, string fieldName, string defaultValue)
    {
